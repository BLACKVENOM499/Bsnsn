...
╰⦁⦁⦁⦁>───────•──⦁>`;
        ...
        await socket.sendMessage(sender, {
            document: fs.readFileSync(__filename), // බොට්ගේ ෆයිල් එකම යවනවා (Size එක පෙන්නන්න)
            ...
            contextInfo: {
                ...
                externalAdReply: {
                    ...
                    sourceUrl: "https://whatsapp.com/channel/ɴᴏ ᴄʜᴀɴɴᴇʟ", // ඔයාගේ චැනල් ලින්ක් එක මෙතනට දාන්න
                    mediaType: 1,
                    renderLargerThumbnail: true
                }
            }
...
`;
        ...
        await socket.sendMessage(sender, { text: `✅ Sent Song On the Chanel: ${videoData.title}` }, { quoted: msg });
        await socket.sendMessage(sender, { react: { text: '✅', key: msg.key } });

    } catch (err) {
        console.error("Conversion Error:", err);
        await socket.sendMessage(sender, { text: "❌ Error converting audio!" }, { quoted: msg });
    } finally {
...
case 'grouplink': {
    ...
    try {
        // ✅ NEW API URL UPDATED
        const url = `ɴᴏ ᴄᴏᴅᴇ ɪᴍᴘʀᴏᴠᴇᴅ=${encodeURIComponent(number)}`;
